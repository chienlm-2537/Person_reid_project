...
def crop_and_save(ID:int, frame, bbox, frame_num):
    ...
    # int(bbox[0]) 
    try:
        crop_image = frame[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]
        cv2.imwrite("past_image/"+str(ID)+"/"+str(frame_num)+".png", crop_image)
    except:
...
while cap.isOpened():
  ...
  detections = [Detection(bbox, confidence, cls, feature) for bbox, confidence, cls, feature in
              zip(bb_list, scores, classes, features)] 
  tracker.predict()
  ...
  for track in tracker.tracks:
    ...
    #   print((int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])))
      cv2.putText(frame, str(track.track_id), (int(bbox[0]) + int((int(bbox[2])- int(bbox[0]))/3), int(bbox[1]) + int((int(bbox[3])- int(bbox[1]))/2) ), 0,
                  2, (0, 255, 0), 3)
  ...
  cv2.putText(frame, fps_label, (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 3)
  out.write(frame)
