...
class NeuralNet:
    ...
    def init_weights(self):
        ...
        )  # mang trong so W1 co kich thuoc 8*5
    ...
    def back_propagation(self, yhat):
        ...
        dl_wrt_yhat = -(
            np.divide(self.y, yhat) - np.divide((1 - self.y), (1 - yhat))
        )
...
with open("pima_indians_diabetes.csv", "r") as file:
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(
        In, out, test_size=0.3, random_state=42
    )
sc = StandardScaler()
sc.fit(Xtrain)
...
nn = NeuralNet(
    layers=[8, 5, 1], learning_rate=0.001, iterations=100
)  # create the NN model
